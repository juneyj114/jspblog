...
public class CommentDAO {
	...
	public List<Comment> findByBoardId(int boardId) {
		final String SQL = "select c.id, c.userId, c.boardId, c.content, c.createDate, u.username, u.userProfile from comment c, user u where c.userId = u.id and boardId = ? and c.commentId is null";
	...
	synchronized public Comment findByMaxId() {
		final String SQL = "select c.id, c.userId, c.boardId, c.content, c.createDate, u.username, u.userProfile from comment c, user u where c.userId = u.id and c.id = (select max(id) from comment)";
		Comment comment = new Comment();
		conn = DBConn.getConnection();
		try {
			pstmt = conn.prepareStatement(SQL);
	...
	public List<Comment> show(int commentId) {
		final String SQL = "select f.id, f.userId, f.boardId, f.content, f.createDate, f.commentId, u.username, u.userProfile from comment f, comment p, user u where f.commentId=p.id and f.userId = u.id and f.commentId=?";
		conn = DBConn.getConnection();
		List<Comment> comments = new ArrayList<Comment>();
		try {
			pstmt = conn.prepareStatement(SQL);
