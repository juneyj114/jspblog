...
async function showReply(commentId) {
  ...
  async function postReply(event, commentId){
      ...
      if(status === "OK"){
        const div = document.createElement("div");
        div.innerHTML = "<div style='margin-left: 50px;' class='comment-list' id='reply-"+id+"'><div class='single-comment justify-content-between d-flex'><div class='user justify-content-between d-flex'><div class='thumb'><img src='"+userProfile+"' alt='' style='height:50px; width:50px;'></div><div class='desc'><h5><a href='#'>"+username+"</a></h5><p class='date'>"+createDate+"</p><p class='comment'>"+content+"</p></div></div><div class='reply-btn'><button class='btn-reply text-uppercase' onclick='deleteReply("+id+")'>❌</button></div><div class='reply-btn'><button class='btn-reply text-uppercase'>댓글보기</button><button id='btnWriteComment-"+id+"' class='btn-reply text-uppercase' onclick='writeReply("+id+")'>댓글쓰기</button></div></div></div>";
        console.log(div);
        $("#childReply-"+commentId).append(div);
  ...
  async function writeComment(event) {
      ...
      if(status === "OK"){
        const div = document.createElement("div");
        div.innerHTML = "<div class='comment-list' id='comment-"+id+"'><div id = 'reply-"+id+"' class='single-comment justify-content-between d-flex'><div class='user justify-content-between d-flex'><div class='thumb'><img src='"+userProfile+"' alt=''style='width: 50px; height: 50px;'></div><div class='desc'><h5><a href='#'>"+username+"</a></h5><p class='date'>"+createDate+"</p><p class='comment'>"+content+"</p></div></div><div class='reply-btn'><button class='btn-reply text-uppercase' onclick='deleteReply("+id+")'>❌</button></div><div class='reply-btn'><button class='btn-reply text-uppercase'>댓글보기</button><button id='btnWriteComment-"+id+"' class='btn-reply text-uppercase' onclick='writeReply("+id+")'>댓글쓰기</button></div></div></div>";
        console.log(div);
